@php
    $sub_options = \App\Actions\Sidebars\GetTopicsSidebaroptions::run($project_id);
...
@if(count($sub_options) > 0)
    <ul class="nav-dropdown-items">
        @foreach($sub_options as $sub_option)
            <li class="nav-item">
                <a class="nav-link" @if(!is_null($sub_option['listing_route']))href="{!! url($sub_option['listing_route']) !!}?topic={!! $sub_option['id'] !!}"@endif>
