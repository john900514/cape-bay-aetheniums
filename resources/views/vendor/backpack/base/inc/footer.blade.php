@if (config('backpack.base.show_powered_by') || config('backpack.base.developer_link'))
    <div class="text-muted ml-auto mr-auto">
      ...
      {{ trans('backpack::base.handcrafted_by') }} <a target="_blank" rel="noopener" class="text-primary" href="{{ config('backpack.base.developer_link') }}">{{ config('backpack.base.developer_name') }}</a>.
      @endif
